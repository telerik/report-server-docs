...
![Setup Storage ><](../../images/report-server-images/setup-storage-step-one-report-server.png)
...
The storage settings go into the Report Server configuration file called _ReportServerAdmin.config_. The Report Server installer preserves that file upon upgrade. That way the setting will remain when you upgrade the Report Server version.
...
![Setup Storage ><](../../images/report-server-images/setup-storage-step-two-report-server.png)

## Step 3: Configure Encryption

Sensitive assets are stored in the Report Server Storage in encrypted form using industry-standard encryption routines. The encryption process relies on two keys - **Main Key** and **Backup Key** - created during the initial configuration of the Report Server.

![Setup Storage ><](../../images/report-server-images/setup-storage-step-three-report-server.png)
