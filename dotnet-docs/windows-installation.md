...
1. Next, the RS.NET checks its `appsettings.json` configuration file for the key **InitialAgentUrl**. If the installation has passed successfully, the key must exist and must have a valid value like **http://localhost:84**. This is where the MSI installation file for Windows has registered the **RS.NET Service Agent**.
