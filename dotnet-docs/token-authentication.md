...
* If you reinstall RS.NET without removing the storage or migrate the storage from one RS.NET to another the existing Tokens will stop working because they are signed with a different key. The Tokens will still be visible in the UI as they exist in the storage. We are working on imporoving this behavior.
